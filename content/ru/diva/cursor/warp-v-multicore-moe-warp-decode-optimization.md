...
В статье "Better MoE model inference with Warp Decode" компания Cursor представила инновационный подход к ускорению инференса моделей смешения экспертов (MoE) для **low-batch-сценариев**, достигнув ускорения **1.84×** на NVIDIA B200. Их метод, названный Warp Decode, меняет логику распараллеливания: **вместо того чтобы распределять экспертов по варпам, они назначают каждый варп на вычисление конкретного выходного нейрона**.
